...
.customSanitizer((value) => {
    // Convierte la cadena separada por comas en un array, eliminando espacios adicionales
    if (typeof value === 'string') {
      return value.split(',').map(poder => poder.trim());
    }
    return value; // Devuelve el valor original si ya es un array
  })
.isArray({ min: 1 })
.withMessage('Debe proporcionar al menos un poder')
//custum(callback) Para definir validaciones especificas 
.custom((poderes)=>{
...
.custom((aliados)=>{
    for(const aliado of aliados){
        ...
        if (aliado.trim().length < 3 || aliado.trim().length > 60) {
            throw new Error("Cada aliado debe contener entre 3  y 60 caracteres");
        }
    }
    return true
})
...
.custom((enemigos)=>{
    for(const enemigo of enemigos){
        if (typeof enemigo !== 'string') {
            throw new Error("Todos los enemigo deben ser cadenas de texto");
        }
        if (enemigo.trim().length < 3 || enemigo.trim().length > 60) {
            throw new Error("Cada enemigo debe contener entre 3  y 60 caracteres");
        }
    }
    return true
})
